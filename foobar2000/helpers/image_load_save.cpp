...
namespace fb2k {
	bool imageSaveDialog(album_art_data_ptr content, HWND wndParent, const char* initDir, bool bAsync) {
		...
		if (!fb2k::getOpenFileName(wndParent, fileTypes, 0, ext.length() > 0 ? ext.c_str() : nullptr, "Export picture file", initDir, fn, TRUE)) return false;
	...
	bool imageLoadDialog(pfc::string_base& outFN, HWND wndParent, const char* initDir) {
		return fb2k::getOpenFileName(wndParent, FB2K_GETOPENFILENAME_PICTUREFILES_ALL, 0, nullptr, "Import picture file", initDir, outFN, FALSE);
	}
