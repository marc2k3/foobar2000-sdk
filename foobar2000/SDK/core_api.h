...
namespace core_api {
	...
	//! Your component should not display any forms of user interface when running in quiet mode, as well as avoid saving configuration on its own (no need to worry if you only rely on @c cfg_vars or @c config_io_callback, they will simply be ignored on shutdown).
	bool is_quiet_mode_enabled();
};

#define FB2K_SUPPORT_LOW_MEM_MODE 1
...
namespace fb2k {
	...
	using core_api::is_main_thread;
	using core_api::pathInProfile;
	using core_api::get_profile_path;
	using core_api::is_shutting_down;
	using core_api::is_initializing;
	using core_api::are_services_available;
	using core_api::is_portable_mode_enabled;
	using core_api::is_quiet_mode_enabled;
	using core_api::get_main_window;
}
