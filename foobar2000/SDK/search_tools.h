...
//! Use @c search_filter_manager API to instantiate search_filter objects.
...
public:
	...
	//! Abortable version of @c test_multi(). If the @c abort_callback object becomes signaled while the operation is being performed, contents of the output buffer are undefined and the operation will fail with @c exception_aborted.
...
public:
	//! Returns whether the sort pattern returned by @c get_sort_pattern() was set by the user explicitly using "SORT BY" syntax or whether it was determined implicitly from some other part of the query. It's recommended to use this to determine whether to create a force-sorted autoplaylist or not.
...
//! \since 0.9.5
//! Entrypoint class to instantiate @c search_filter objects.
class search_filter_manager : public service_base {
public:
	//! Creates a @c search_filter object. Throws an exception on failure (such as an error in the query). It's recommended that you relay the exception message to the user if this function fails.
...
public:
	...
	/// Creates a @c search_filter object. Throws an exception on failure (such as an error in the query). It's recommended that you relay the exception message to the user if this function fails. \n
    /// Creation of self-refreshing queries, such as system time dependant ones, must be issued from main thread only. \n
    /// Pass KFlagSuppressNotify to allow such queries to be created off-main-thread, but notifications will not be called.
	//! @param changeNotify A @c completion_notify callback object that will get called each time the query's behavior changes as a result of some external event (such as system time change). The caller must refresh query results each time this callback is triggered. The status parameter of its @c on_completion() parameter is unused and always set to zero.
...
public:
	...
	//! @returns list of @c metadb_handle objects. Safe to use @c arr->as_list_of<metadb_handle>() to get a @c pfc::list_base_const_t<metadb_handle_ptr>
	//! @param subset Optional: pass subset of tracks in this index to search - whole index is searched if nullptr is passed.
	//! @param flags Optional: set @c flag_sort to sort output
