...
//! Implementing this interface lets you maintain your own configuration files rather than depending on the @c cfg_var system. \n
//! Note that you must not make assumptions about what happens first: @c config_io_callback::on_read(), initialization of @c cfg_var values or @c config_io_callback::on_read() in other components. Order of these things is undefined and will change with each run. \n
//! Use @c service_factory_single_t<myclass> to register your implementations. Do not call other people's implementations, core is responsible for doing that when appropriate.
class NOVTABLE config_io_callback : public service_base {
public:
	//! Called on startup. You can read your configuration file from here. \n
	//! Hint: use @c core_api::get_profile_path() to retrieve the path of the folder where foobar2000 configuration files are stored.
...
public:
	//! Implement optionally. Called to quickly flush recent configuration changes. If your instance of @c config_io_callback needs to perform timeconsuming tasks when saving, you should skip implementing this method entirely.
...
//! If you get a @c exception_io_transactions_unsupported, let the caller deal with it - your call will be retried with a regular filesystem instead of a transacted one.
