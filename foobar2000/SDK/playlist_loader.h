...
//! Typically, you call one of standard services such as @c playlist_incoming_item_filter instead of implementing this interface and calling @c playlist_loader methods directly.
...
public:
	...
	//! Receives an item from one of @c playlist_loader functions.
	...
	//! Queries whether @c file_info for specified item is requested. In typical scenario, if @c want_info() returns false, @c on_entry() will be called with same parameters; otherwise caller will attempt to read info from the item and call @c on_entry_info() with same parameters and @c file_info read from the item.
	//! @param p_item Item location, in form of @c metadb_handle pointer.
	//! @param p_type Origin of this item - see @c t_entry_type for more info.
	//! @param p_stats File stats of this item; set to @c filestats_invalid if not available.
	//! @param p_fresh Fresh flag; indicates whether stats are directly from filesystem (true) or as stored earlier in a playlist file (false).
	virtual bool want_info(const metadb_handle_ptr & p_item,t_entry_type p_type,const t_filestats & p_stats,bool p_fresh) = 0;
	//! Receives an item from one of @c playlist_loader functions; including @c file_info data. Except for @c file_info to be typically used as hint for metadb backend, behavior of this method is same as `on_entry()`.
	//! @param p_item Item location, in form of @c metadb_handle pointer.
	//! @param p_type Origin of this item - see @c t_entry_type for more info.
	//! @param p_stats File stats of this item; set to @c filestats_invalid if not available.
	//! @param p_info Information about the item, read from the file directly (if @c p_fresh is set to true) or from e.g. playlist file (if @c p_fresh is set to false).
	//! @param p_fresh Fresh flag; indicates whether stats are directly from filesystem (true) or as stored earlier in a playlist file (false).
	...
	//! Returns whether further @c on_entry() calls for this file are wanted. Typically always returns true, can be used to optimize cases when directories are searched for files matching specific pattern only so unwanted files aren't parsed unnecessarily.
...
//! \since 1.3
//! Extended version of @c playlist_loader_callback, allowing caller to pass pre-made @c metadb_info_container \n
...
//! Note that you should typically call @c playlist_incoming_item_filter methods instead of calling @c playlist_loader methods directly to get a list of playable items from a specified path; this way you get a core-implemented threading and abortable dialog displaying progress.\n
//! To register your own implementation, use @c playlist_loader_factory_t template.\n
//! To call existing implementations, use static helper methods of @c playlist_loader class.
class NOVTABLE playlist_loader : public service_base {
public:
	//! Parses specified playlist file into list of playable locations. Throws @c exception_io or derivatives on failure, @c exception_aborted on abort. If specified file is not a recognized playlist file, @c exception_io_unsupported_format is thrown.
	...
	//! Writes a playlist file containing specific item list to specified file. Will fail (`pfc::exception_not_implemented`) if specified @c playlist_loader is read-only (@c can_write() returns false).
	...
	//! Returns extension of file format handled by this @c playlist_loader implementation (a UTF-8 encoded null-terminated string).
	virtual const char * get_extension() = 0;
	//! Returns whether this @c playlist_loader implementation supports writing. If @c can_write() returns false, all write() calls will fail.
	virtual bool can_write() = 0;
	//! Returns whether specified content type is one of playlist types supported by this @c playlist_loader implementation or not.
	...
	//! Attempts to load a playlist file from specified filesystem path. Throws @c exception_io or derivatives on failure, @c exception_aborted on abort. If specified file is not a recognized playlist file, @c exception_io_unsupported_format is thrown. \n
	...
	//! Saves specified list of locations into a playlist file. Throws @c exception_io or derivatives on failure, @c exception_aborted on abort.
	...
	//! Does not touch playlist files encountered - use @c g_process_path_ex() if specified path is possibly a playlist file; playlist files found inside directories or archives are ignored regardless.\n
	...
	//! Calls attempts to process specified path as a playlist; if that fails (i.e. not a playlist), calls @c g_process_path with same parameters. See @c g_process_path for parameter descriptions. \n
