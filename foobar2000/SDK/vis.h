...
//! This class provides abstraction for retrieving visualisation data. Instances of @c visualisation_stream being created/released serve as an indication for visualisation backend to process currently played audio data or shut down when there are no visualisation clients active.\n
//! Use @c visualisation_manager::create_stream to instantiate.
...
public:
	...
	//! @param p_chunk Receives spectrum data. @c audio_chunk type is used for consistency, since required functionality is identical to provided by @c audio_chunk ; the data is *not* PCM. Returned sample count is equal to half of FFT size; channels and sample rate are as in audio stream the spectrum was generated from.
	//! @param p_offset Timestamp of spectrum to retrieve. See @c get_absolute_time().
	...
	//! Generates fake audio chunk to display when @c get_chunk_absolute() fails - e.g. shortly after @c visualisation_stream creation data for currently played audio might not be available yet. \n
	//! Throws @c std::exception derivatives on failure.
	virtual void make_fake_chunk_absolute(audio_chunk & p_chunk,double p_offset,double p_requested_length) = 0;
	//! Generates fake spectrum to display when @c get_spectrum_absolute() fails - e.g. shortly after @c visualisation_stream creation data for currently played audio might not be available yet. \n
	//! Throws @c std::exception derivatives on failure.
...
//! Entrypoint service for visualisation processing; use this to create @c visualisation_stream objects that can be used to retrieve properties of currently played audio. \n
//! Implemented by core; do not reimplement.\n
//! Use @c visualisation_manager::get() to obtain an instance, e.g. @c visualisation_manager::get()->create_stream(mystream,0);
class NOVTABLE visualisation_manager : public service_base {
	FB2K_MAKE_SERVICE_COREAPI(visualisation_manager);
public:
	//! Creates a @c visualisation_stream object. See @c visualisation_stream for more info.
	//! @param p_out Receives newly created @c visualisation_stream instance.
	//! @param p_flags Combination of one or more @c KStreamFlag* values. Currently only @c KStreamFlagNewFFT is defined.
	//! It's recommended that you set p_flags to @c KStreamFlagNewFFT to get the new FFT behavior (better quality and result normalization), the old behavior for null flags is preserved for compatibility with old components that rely on it.
	...
	//! Wrapper around non-template @c create_stream(); retrieves one of newer @c visualisation_stream_* interfaces rather than base @c visualisation_stream interface. Throws @c exception_service_extension_not_found() when running too old foobar2000 version for the requested interface.
