...
//! Entrypoint interface for declaring component's version information. Instead of implementing this directly, use @c DECLARE_COMPONENT_VERSION().
...
//! Implementation helper. You typically want to use @c DECLARE_COMPONENT_VERSION() instead.
...
//! You should have only one @c DECLARE_COMPONENT_VERSION() per component DLL. Having more than one will confuse component updater's version matching. \n
...
//! Same as @c DECLARE_COMPONENT_VERSION(), but parameters can be dynamically generated strings rather than compile-time constants.
...
#ifdef _WIN32
//! Simple implementation of @c component_installation_validator that makes sure that our component DLL has not been renamed around by idiot users.
