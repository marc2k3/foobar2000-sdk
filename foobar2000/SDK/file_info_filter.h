...
//! Implementing this class gives you direct control over which part of @c file_info gets altered during a tag update uperation. To be used with `metadb_io_v2::update_info_async()`.
class NOVTABLE file_info_filter : public service_base {
	FB2K_MAKE_SERVICE_INTERFACE(file_info_filter, service_base);
public:
	//! Alters specified @c file_info entry; called as a part of tag update process. Specified @c file_info has been read from a file, and will be written back.\n
	//! WARNING: This will be typically called from another thread than main app thread (precisely, from thread created by tag updater). You should copy all relevant data to members of your @c file_info_filter instance in constructor and reference only member data in @c apply_filter() implementation.
	//! @returns True when you have altered @c file_info and changes need to be written back to the file; false if no changes have been made.
    ...
    static file_info_filter::ptr create_overwriter( const file_info & );
};

//! Extended @c file_info_filter allowing the caller to do their own manipulation of the file before and after the metadata update takes place. \n
//! Respected by foobar2000 v1.5 and up; if @c metadb_io_v4 is supported, then @c file_info_filter_v2 is understood.
...
public:
	...
	//! Called after just before rewriting metadata. The file is not yet opened for writing, but a @c file_lock has already been granted (so don't call it on your own). \n
	//! You can use this method to perform album art updates (via @c album_art_editor API) alongside metadata updates. \n
	...
	//! If you wish to alter the file on your own, use @c before_tag_update() for this instead. \n
	//! If your own operations on this file fail, just pass the exceptions to the caller and they will be reported just as other tag update errors. \n
