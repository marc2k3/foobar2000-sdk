...
class dsp_manager {
public:
    //! @param creationFlags See @c dsp_entry::flag_*
    ...
    void set_config( auto && arg ) {
        m_config = std::forward<decltype(arg)>(arg);
        m_config_changed = true;
    }
    
	//! Runs DSP on the specified chunk list.
	//! @param p_flags See: @c dsp::run() flags,
	...
	//! Equivalent to @c set_config() with empty configuration.
	void close();

	//! Returns whether there's at least one active DSP in the configuration.
	bool is_active() const;
	//! Returns whether at least one active DSP wants to know exact track change moment.
...
//! Use @c dsp_config_manager::get() to obtain an instance.
...
public:
	...
	//! @param p_id Control ID of the embedded dialog. The parent window will receive a @c WM_COMMAND with @c BN_CLICKED and this identifier when user changes settings in the embedded dialog.
	...
	//! Retrieves current settings from an embedded DSP settings dialog. See also: @c configure_embedded().
	virtual void configure_embedded_retrieve(fb2k::hwnd_t wnd,dsp_chain_config & p_data) = 0;
	//! Changes current settings in an embedded DSP settings dialog. See also: @c configure_embedded().
...
//! \since 2.25
class dsp_config_manager_v3 : public dsp_config_manager_v2 {
	FB2K_MAKE_SERVICE_COREAPI_EXTENSION(dsp_config_manager_v3, dsp_config_manager_v2);
public:
	//! Acquire and retain the object for the duration your component is doing live editing of DSP. \n
	//! This will hint playback code to reduce latency.
	virtual service_ptr acquire_live_edit_scope() = 0;
	virtual bool test_live_edit_scope() = 0;
};
